...
fun CouiKernelSUTheme(
    ...
    val couiPaletteStyle = try {
        ThemePaletteStyle.valueOf(colorStyle.name)
    } catch (_: Exception) {
        ThemePaletteStyle.TonalSpot
    }

    val couiColorSpec = if (colorSpec.effectiveFor(colorStyle) == ColorSpec.SpecVersion.SPEC_2025) {
    ...
    COUITheme(
        ...
        content = {
            ...
            CompositionLocalProvider(
                LocalContentColor provides COUITheme.colorScheme.onBackground,
            ) {
                content()
            }
        }
